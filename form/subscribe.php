...
$emailTO[] = array( 'email' => 'info@wife-doge.tech', 'name' => 'HA' ); 
	...
	if (isset($_POST["contact-email"]) && $_POST["contact-email"] != '') {
		...
		if ($honeypot == '' && !(empty($emailTO))) {
			### If you want use SMTP 
			 ...
			 $mail->Port = 465;
			 $mail->SMTPAuth = true;
			 $mail->Username = 'info@wife-doge.tech';
			 $mail->Password = 'smoemoe';
			...
			$bodymsg .= $_SERVER['HTTP_REFERER'] ? '<br>---<br><br>This email was sent from [WifeDoge]: ' . $_SERVER['HTTP_REFERER'] : '';
		...
		echo json_encode(array ('result' => "error", 'message' => "Please <strong>Fill up</strong> a correct email and try again."));
	}
}